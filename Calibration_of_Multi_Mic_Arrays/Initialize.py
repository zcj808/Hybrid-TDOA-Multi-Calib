...
def get_initial_value(g,category):
    ...
    # # if dataset != 0: # means TDOA is pre-generate from file
    ...
    # mat_data = sio.loadmat(f"zhangSimDataset/g_list{dataset}.mat")
    ...
    for mic_lie_a in mic_lie_a_gt:
        ...
        euler_angles = rotationMatrixToEulerAngles(rot.as_matrix())/np.pi*180
        # euler_angles = rot.as_euler('xyz', degrees=True) # zyx?
        mic_angle_gt.append(euler_angles) # Check Over
    ...
    record_time2=[record_time[0]]
    rt=record_time[0]
    for record_t in record_time[1:]:
        rt=rt+record_t
        record_time2.append(rt)
    record_time=np.array(record_time2)
    sound_speed=340
    interval=10
    fig=False
    TDOA_FIG=False
    ...
    ID = []
    tdoa_mea=g['tdoa_m'].T
    tdoa_mea=np.hstack((np.zeros((K,1)),tdoa_mea))

    doa=g['doa']
    doa_mea=-np.array(doa)

    odo_mea=g['odo'].T
    odo_mea=np.vstack((odo_mea,np.zeros((1,3))))
...
# -----------ZHANG FIX BELOW-------------#  
    ...
    for i in range(time_steps):
        ...
        T_i_k   = tdoa_mea[i]
        d_i_k = doa_mea[i]
        # if dataset == 0: # means TDOA generate randomly
    ...
    # 1. Estimation of sound source position (initial value est. method)
    #  triangulation
