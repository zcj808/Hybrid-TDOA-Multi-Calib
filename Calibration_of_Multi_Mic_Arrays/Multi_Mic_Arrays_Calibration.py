...
def covariance_matrix(mic_num,category):
    TDOA_var_inv = 1/zhangTDOASigma**2
    DOA_var_inv = 1111
    if category=="Simulation":
        if zhangDOASigma==5:
            DOA_var_inv = 600
        elif zhangDOASigma==10:
            DOA_var_inv = 100
        elif  zhangDOASigma==15:
            DOA_var_inv = 70
        else:
            DOA_var_inv = 500
    else:
        DOA_var_inv = 50
    # DOA_var_inv optimal parameters: sigma=5,10,15 inv=500,100,
...
def linearize_and_solve_with_H(x,measures,ID,mic_num,interval,category,record_time):
    ...
    omiga_P = np.diag([1/zhangOdoSigma**2, 1/zhangOdoSigma**2, 1/zhangOdoSigma**2])
    # True --> P    False --> L
    P_L = False
...
def estimation(g_list):
    data_id=range(len(g_list[0])) # Assume repeat 100 times under one dataset
    wang_rec=[]
    init_cost = []
    calib_cost = []
    fail_count=0
    for k in data_id:
        ...
        g=g_list[0,k] # extract data of one run
        global zhangDOASigma
        zhangDOASigma=g['doa_sigma'][0][0]
        global zhangTDOASigma
        zhangTDOASigma=g['tdoa_sigma'][0][0]
        global zhangOdoSigma
        zhangOdoSigma=g['odo_sigma'][0][0]
        print(f"data id {k+1}")
        # A. Initial Value Selection
        init_start = time.time()
        x, measures, ID, mic_num, x_gt, interval,record_time  = get_initial_value(g,category)
        init_end = time.time()
        ...
        # results
        ini_norm_mic_pos = norm(ini_delta_mic_pos) / np.sqrt(len(ini_delta_mic_pos)/3)
        ...
        ini_norm_src_pos = norm(ini_delta_src_pos) / np.sqrt(len(ini_delta_src_pos)/3)
        # print("RMSE of  Initial Values:",ini_norm_mic_pos, ini_norm_mic_theta, ini_norm_mic_tau, ini_norm_mic_delta, ini_norm_src_pos)

        fin_norm_mic_pos = norm(fin_delta_mic_pos) / np.sqrt(len(fin_delta_mic_pos)/3) # modify
        ...
        fin_norm_src_pos = norm(fin_delta_src_pos) / np.sqrt(len(fin_delta_src_pos)/3) # modify
        # print("RMSE of Estimated Values:",round(fin_norm_mic_pos,3), round(fin_norm_mic_theta,2), round(fin_norm_mic_tau,6), round(fin_norm_mic_delta,7), round(fin_norm_src_pos,3))
        # print("fail rate: {}%".format(fail_count/len(dataset)*100))
        wang_rec.append(np.array([k+1,ini_norm_mic_pos,ini_norm_mic_theta,ini_norm_mic_tau,ini_norm_mic_delta,ini_norm_src_pos,\
                                fin_norm_mic_pos,fin_norm_mic_theta,fin_norm_mic_tau,fin_norm_mic_delta,fin_norm_src_pos]))
    wang_rec=np.array(wang_rec)
    return wang_rec


if  __name__ == "__main__":
    category='Real_world' # 'Simulation' OR "Real_world"
    category,numIterations, epsilon, est_fig,display_norm_dx_on = optimal_config(category)
    ...
    if category=='Simulation':
        TDOASigmas=[5e-5,1e-4,5e-4] # wang's definition has been modified
        DOASigmas = [5,10,15]
        for i,DOASigma in enumerate(DOASigmas):
            for j,TDOASigma in enumerate(TDOASigmas):
                index=3*i+j+1
                zhangDataset = sio.loadmat(f"../Dataset/sim{index}.mat")
                g_list=zhangDataset['g_list']
                wang_rec=estimation(g_list)
                sio.savemat(f"../Outputs/wang_rec{index}.mat", {"wang_rec": wang_rec})
    elif category=="Real_world":
        zhangDataset = sio.loadmat(f"../Dataset/real.mat")
        g_list=zhangDataset['g_list']
        wang_rec=estimation(g_list)
        sio.savemat(f"../Outputs/real_wang_rec.mat", {"real_wang_rec": wang_rec})
