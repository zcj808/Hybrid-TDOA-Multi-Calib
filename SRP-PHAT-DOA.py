...
if __name__ == "__main__":
    fs=16000
    # Define locations of microphone in array
    mics = torch.zeros((6, 3))
    angle = torch.Tensor([60, 120, 180, -120, -60, 0]) / 180 * np.pi
    rou = 70.85 / 1000 / 2 # radius of the microphone array
    for i in range(len(mics)): # polar coordinates
        mics[i, :] = torch.Tensor([rou * torch.cos(angle[i]), rou * torch.sin(angle[i]), 0])


    # Initialize short-time Fourier transform (STFT) and convariance matrix
    ...
    for i in range(exp_num):
        ...
        audio_ids = audio_ids['arr_arr_id'][0]
        scale_chirp_len=int(0.6*fs)
        our_doas=[]
        for i in range(arr_num):
            our_doa=[]
            # print(doa_audios["data"][i].shape)
            for j in range(K):
                start_id=audio_ids["id"][i][0,j]
                doa_audio=doa_audios["data"][i][:,start_id-1000:start_id-1000+scale_chirp_len].T    
                wav_datas=np.array([doa_audio]).astype(np.float32)
                Xs = stft(torch.from_numpy(wav_datas))
                XXs = cov(Xs) # compute covariance matrix
                srpphat = SrpPhat(mics=mics, sample_rate=fs, speed_sound=346) # initialize srpPhat
                # Use srpPhat to compute direction of source
                doas =  srpphat(XXs)
                doas = doas[:, 0, :]
                # postprocessing
                doas[:, 2] = doas[:, 2].abs()
                ...
                doa=np.array(doas)
                doa=doa[0,:]
                our_doa.append(doa) # dir_rec=(s_loc-mic_loc)/norm(s_loc-mic_loc)
            our_doas.append(our_doa)
            print("------")
            # exit()
        our_doas=np.array(our_doas)
        doa = {'doa': our_doas}
        savemat(f"data\\{exp_id}\\doa.mat", doa)